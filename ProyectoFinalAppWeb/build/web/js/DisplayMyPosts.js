...
class DisplayMyPosts {

    constructor() {
        if (sessionStorage.getItem('usr')) {
            fetch('/ProyectoFinalAppWeb/MisPostsServlet?nombreUsuario=' + JSON.parse(sessionStorage.getItem('usr')).nombreCompleto + '&esAdmin=' + JSON.parse(sessionStorage.getItem('usr')).esAdmin)
    ...
    desplegarPostsComunes(postsComunes, comentarios) {
        ...
        for (var item = 0; item < postsComunes.length; item++) {
            ...
            if (sessionStorage.getItem('usr')) {
                ...
                //liMod.innerHTML = "<a href=\"" + postsComunes[item].id + "\"><button class=\"btnBlue\">Editar</button></a>";
                ...
                aEdit.setAttribute("href", postsComunes[item].id);
                aEdit.innerHTML = "<button class=\"btnBlue\">Editar</button>";

                aEdit.onclick = function (evt) {
                    evt.preventDefault();
                    sessionStorage.setItem("idPostMod", aEdit.getAttribute("href"));
                    window.location.replace('./Edit.jsp');
                };

                liEliminar.appendChild(aEliminar);
                liMod.appendChild(aEdit);

                ulEliminar.appendChild(liEliminar);
                ulEliminar.appendChild(liMod);
        ...
        if (sessionStorage.getItem('usr')) {
            for (var item = 0; item < postsAnclados.length; item++) {
                ...
                if (sessionStorage.getItem('usr')) {
                    ...
                    let aEdit = document.createElement("a");
                    aEdit.setAttribute("href", postsAnclados[item].id);
                    aEdit.innerHTML = "<button class=\"btnBlue\">Editar</button>";
